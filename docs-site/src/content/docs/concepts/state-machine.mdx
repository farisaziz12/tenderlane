...
import StateMachineDiagram from '../../../components/diagrams/StateMachineDiagram.astro';
import CheckoutFlowComparisonDiagram from '../../../components/diagrams/CheckoutFlowComparisonDiagram.astro';
...
## Inline vs. redirect flows

The state machine fans out into two distinct flows depending on the route's `flow` field. Inline flows (`payment-intent`) take an extra `prepare` → `prepared` hop to create a server session before submission; redirect flows (`checkout-session`, `redirect`) skip straight to `submit` and hand control to the PSP's hosted page.

<CheckoutFlowComparisonDiagram />
