...
import ServerHandlerFlowDiagram from '../../../components/diagrams/ServerHandlerFlowDiagram.astro';
...
A single `POST` to the handler runs through six steps. The defensive `stripClientAmounts()` call and the per-provider catalog dispatch are the load-bearing pieces — they're why the server can trust the resolved amounts no matter what the client sent.

<ServerHandlerFlowDiagram />
